...
interface TouchEventCustom extends TouchEvent {
  pointerType: string;
  pageX: number;
  pageY: number;
  buttons: number;
...
export default function(i: DataInterface) {
  if ((!env.supportsTouch && !env.supportsIePointer) || i.element === null) {
    return;
  }
  ...
  function shouldPrevent(deltaX: number, deltaY: number): boolean {
    ...
    if (magnitudeY > magnitudeX) {
      ...
      if (
        (deltaY < 0 && scrollTop === i.content.height - i.container.height) ||
      ...
      if (
        (deltaX < 0 && scrollLeft === i.content.width - i.container.width) ||
  ...
  function shouldHandle(e: TouchEventCustom): boolean {
    ...
    if (
      e.pointerType &&
      e.pointerType !== "mouse" &&
      e.pointerType !== (<any>e).MSPOINTER_TYPE_MOUSE
    ) {
      return true;
    }
  ...
  function touchStart(e: TouchEventCustom): void {
    if (!shouldHandle(e)) {
      return;
    }
  ...
  function touchMove(e: TouchEventCustom): void {
    if (shouldHandle(e)) {
      const touch = getTouch(e);
